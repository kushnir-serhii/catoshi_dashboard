...
| 024 | Today Range & Level Probability | **Gated, in progress** — pure model, backtest script + `today-range-backtest.yml`, `/api/today`, `TodayPanel` (hidden until `TODAY_GATE_VERDICT` is A/B) and daily scoring (`0011_today_range_predictions.sql`, collect stage) shipped in code; the real backtest run (§0 verdict, fitted `k`), migration apply, and 1440px/375px check pending operator |
