...
| 020 | Scheduled Forecast Ingestion + On-Demand Fallback | **In progress** — all five slices' code shipped (`GET /api/projections/inputs` + `POST /api/projections/ingest` behind `FORECAST_INGEST_SECRET`, `validateIngestBatch` grid/probability/anchor guards, `source='routine'`/`cost_usd=0` rows through `persistForecasts`, `producer` on `ProjectionsResponse` + the Projections page label, `/api/health` `forecast_ingest` state, `docs/routine-forecast-prompt.md` + runbook §11); `forecast-ingest.test.ts` 53 checks. Pending operator: set `FORECAST_INGEST_SECRET` in Vercel + deploy, the Slice 1 reachability gate (a throwaway scheduled task hitting `/api/projections/inputs`), create the scheduled task from the prompt, and the one-week zero-paid-generation reconciliation |
| 024 | Today Range & Level Probability | **Rejected — Verdict C** (2026-09-23, functional-spec §0): out-of-sample, both horizons, the fitted per-asset `k` narrows the 90% band close to target but cannot fix the 50% band (over-covers 4-14pp) or touch/close-beyond reliability at the same time. Nothing ships. Code (pure model, `/api/today`, `TodayPanel`, daily scoring) is kept but inert — `TODAY_GATE_VERDICT = 'C'` in `src/consts/today.ts` hides the card and stops the collect stage from issuing new predictions |
| 025 | Scenario Simulator Fixes | **In progress** — honesty fixes shipped in code (Low/Median/High percentile labels, drift default 0 with realized drift as reference text, simulator Reforecast button removed, horizon chips, collapsed Assumptions disclosure, lowercase σ); `scenario-stats.test.ts` added; 1440px/375px screenshots pending |
