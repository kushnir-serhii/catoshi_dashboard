...
- **News & Signals:** No CryptoPanic integration ships in code — `CRYPTOPANIC_API_KEY` still exists in `.env.example` but no path in `src/lib/` calls the CryptoPanic API (verified against `src/lib/marketData.ts`; corrected here, spec 010 Slice 7). The actual news input is the RSS-to-JSON bridge below, combined with Reddit sentiment.
