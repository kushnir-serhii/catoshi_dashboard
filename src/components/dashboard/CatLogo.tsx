...
import { LogoVariant } from './context';

const COLOR = 'oklch(0.78 0.22 295)';

function CatC_Tail({ size = 32, glow = 1 }: { size?: number; glow?: number }) {
  return (
    <span className="cat-logo" style={{ filter: `drop-shadow(0 0 ${6 * glow}px oklch(0.65 0.22 295 / ${0.5 * glow}))` }}>
...
function CatC_Mono({ size = 32, glow = 1 }: { size?: number; glow?: number }) {
  return (
    <span className="cat-logo" style={{ filter: `drop-shadow(0 0 ${5 * glow}px oklch(0.65 0.22 295 / ${0.4 * glow}))` }}>
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <path
...
export function Cat({ variant = 'tail', size = 32, glow = 1 }: { variant?: LogoVariant; size?: number; glow?: number }) {
